...
int main(int, char**)
{
	...
	namedWindow("contours",0);
        frame = imread("/home/jinyankai/segment/build/src/test_img.jpg");

        if (!frame.empty()) {
		const auto size = frame.size();

		// check if the size of the frame is zero
		if ((size.width - 1) * (size.height - 1) <= 1) {
			std::cerr << "Error size of the frame \n";
		}
